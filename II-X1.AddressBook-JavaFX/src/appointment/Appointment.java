...
public class Appointment {
	...
	public Appointment() {
		...
		/**
		 * ObjectBinding startDate an startTime Es wird ein LocalDateTime Objekt
		 * erzeugt.
		 */
		startDateTime = new ObjectBinding<LocalDateTime>() {
			...
			protected LocalDateTime computeValue() {
				...
				LocalDateTime timeDateTime = LocalDateTime.parse(
						startDate.get() + " " + startTime.get(),
						dateTimeFormatter);
				return timeDateTime;
			}
		};
		...
		/**
		 * ObjectBinding endDate an endTime Es wird ein LocalDateTime Objekt
		 * erzeugt.
		 */
		endDateTime = new ObjectBinding<LocalDateTime>() {
			...
			protected LocalDateTime computeValue() {
				DateTimeFormatter dateTimeFormatter = DateTimeFormatter
						.ofPattern("MM/dd/yyyy HH:mm");
				LocalDateTime timeDateTime = LocalDateTime.parse(endDate.get()
						+ " " + endTime.get(), dateTimeFormatter);
				return timeDateTime;
			}
		};
		/**
		 * ObjectBinding startDateTime an endDateTime Es wird die Dauer zwischen
		 * den beiden Terminen als Long value generiert.
		 */
		duration = new ObjectBinding<Long>() {
			...
			protected Long computeValue() {
				return startDateTime.get().until(endDateTime.get(),
						ChronoUnit.MINUTES);
			}
		};
	 ...
	 */
	public Appointment(String dateContentIN, String startTimeIN,
			String endTimeIN, String startDateIN, String endDateIN,
			String categoryIN) throws IllegalTimeException, ParseException {
		...
		/**
		 * Wirft IllegalTimeException, wenn der Startzeitpunkt vor dem
		 * Endzeitpunkt liegt
		 */
	...
	/**
	 * Konstruktor (für Liste von String) - erzeugt ein Appointment aus den
	 * durch die CSV/BINAppointmentReader Klasse übergebenen Werte
	 */
	public Appointment(String[] split) throws IllegalTimeException {
		this.dateContent = new SimpleStringProperty(split[0]);
	...
	/**
	 * DateTimeFormatter formatiert das LocalDateTimeObjekt zu einem String nach
	 * dem vorgegeben Muster
	 */
	public String getStartDateTime() {
		return startDateTime.get().format(
	...
	public void setStartDate(String startDate) throws IllegalTimeException,
			ParseException {
	...
	public void setEndDate(String endDate) throws IllegalTimeException,
			ParseException {
	...
	public void setStartTime(String startTime) throws IllegalTimeException,
			ParseException {
	...
	public void setEndTime(String endTime) throws IllegalTimeException,
			ParseException {
	...
	/**
	 * Gibt die Werte, aus dem übergebenen Long Wert in Sekunden, Minuten,
	 * Stunden und Tage (in einem Array) wieder
	 */
	public Long[] getDurationSplit() {
		...
		if (duration.get() >= 60 & duration.get() < 1440) { // < ein Tag
			retArr[1] = duration.get() / 60; // wieviele stunden
			retArr[0] = duration.get() % 60;// restliche Minuten
			return retArr;
		} else {
			retArr[2] = duration.get() / 1440;// wieviele T<ge
			System.out.println(retArr[2]);
			retArr[1] = (duration.get() - (retArr[2] * 1440)) / 60; // Stunden
			retArr[0] = (duration.get() - (retArr[2] * 1440)) % 60;// Minuten
			return retArr;
		}
	}
